...
function pageLogic($previousData){
    ...
    $query = "SELECT members.id, members.lname, members.family_size, members.city, SUM(challenge.d_tri/members.family_size) as d_tri, SUM(challenge.d_verre/members.family_size) as d_verre, SUM(challenge.d_compost/members.family_size) as d_compost, SUM(challenge.d_ordure/members.family_size) as d_ordure FROM challenge RIGHT JOIN members ON challenge.member=members.id AND EXTRACT(MONTH FROM NOW()) = EXTRACT(MONTH FROM challenge.date) WHERE members.role = 'Participant' GROUP BY members.id, EXTRACT(MONTH FROM challenge.date)";
